...
class _AvailableTravelerScreenState extends State<AvailableTravelerScreen> {
  ...
  String _searchValue = '';
  Map<String, double?> userRatings = {}; // Map to store user ratings
  ...
  void fetchTravelers() async {
    ...
    // Fetch ratings for each traveler
    for (var traveler in travelers) {
      double? rating = await fetchUserRating(traveler.userId);
      userRatings[traveler.userId] = rating;
    }
    setState(() {}); // Update the UI after fetching travelers and ratings
  }
  ...
  Future<double?> fetchUserRating(String userId) async {
    final ratingsCollection = FirebaseFirestore.instance.collection('rates');
    final querySnapshot = await ratingsCollection.where('ratedUserID', isEqualTo: userId).get();
    ...
    return totalRating / querySnapshot.docs.length; // Calculate average rating
  }
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        SizedBox(width: 5.0),
                                        Text(
                                          userRatings[traveler.userId] != null
                                              ? userRatings[traveler.userId]!.toStringAsFixed(1)
                                              : 'N/A',
                                          style: TextStyle(
                                            fontSize: 16.0,
                                            color: Colors.black,
